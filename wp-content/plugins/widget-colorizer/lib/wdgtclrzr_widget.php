...
class Wdgtclrzr extends WP_Widget {

	function wdgtclrzr() {
		$widget_ops = array(
			'classname' => 'wdgtclrzr',
			'description' => __( 'Widget Customizer! Customize your next widget.', 'wdgtclrzr' )
		);
		parent::__construct(
			'wdgtclrzr',
			'+ Widget Customizer',
	...
	function widget( $args, $instance ) {
		...
		if ( $curr_preset == 'Custom' ) {
			...
			$loaded_presets = get_option('wdgtclrzr_presets');
			if ( isset($loaded_presets) && !empty($loaded_presets) && is_array($loaded_presets) ) {
				$curr_wc = $loaded_presets[$curr_preset];
			}
		}

		$wc_id = uniqid('wdgtclrzr-');

		$custom_css = '';
		if ( $curr_wc['custom_css'] !== '' ) {
			$curr_classes = array_filter(explode('@widget', $curr_wc['custom_css'] ));
			if ( is_array($curr_classes) ) {
				foreach ( $curr_classes as $class ) {
					$custom_css .= 'html body #'.$wc_id.'+aside'.$class.'html body #'.$wc_id.'+div'.$class.'html body #'.$wc_id.'+section'.$class;
				}
			}
		}
		...
		<?php
			Wdgtclrzr_Engine::$settings['css'] .= sprintf('
				html body #%1$s+aside, html body #%1$s+div, html body #%1$s+section {
					%3$s %6$s %7$s %8$s %19$s %20$s %21$s %22$s %23$s %24$s %25$s %26$s %27$s %28$s %29$s %30$s %31$s %32$s %34$s
				}
				html body #%1$s+aside *, html body #%1$s+div *, html body #%1$s+section * {
					%14$s %15$s %16$s %17$s %18$s
				}
				html body #%1$s+aside a, html body #%1$s+div a, html body #%1$s+section a {
					%4$s
				}
				html body #%1$s+aside a:hover, html body #%1$s+div a:hover, html body #%1$s+section a:hover {
					%5$s
				}
				html body #%1$s+aside h1, html body #%1$s+aside h2, html body #%1$s+aside h3, html body #%1$s+aside h4, html body #%1$s+aside h5, html body #%1$s+aside h6, html body #%1$s+div h1, html body #%1$s+div h2, html body #%1$s+div h3, html body #%1$s+div h4, html body #%1$s+div h5, html body #%1$s+div h6, html body #%1$s+section h1, html body #%1$s+section h2, html body #%1$s+section h3, html body #%1$s+section h4, html body #%1$s+section h5, html body #%1$s+section h6 {
...
?>
		<div id="<?php echo $unique_id; ?>" class="wdgtclrzr-widget" data-mode="widget">
			...
			<div class="wdgtclrzr-textarea">
				...
				<small><?php _e('Use @widget prefix for writing CSS.', 'wdgtclrzr_strings'); ?><br /><?php _e('e.g. @widget li {border-top:1px solid #ccc;}', 'wdgtclrzr_strings'); ?></small>
			</div>
 ...
 * Register Widget
 */
function wdgtclrzr_register_widgets() {
	register_widget( 'Wdgtclrzr' );
}
add_action( 'widgets_init', 'wdgtclrzr_register_widgets' );
