...
class NC_Shortcodes {
	...
	public static function init() {
		...
		add_action( 'newscodes_shortcode_before_woo_poster_loop', __CLASS__ . '::add_image_before_loop' );
	...
	private static function loop( $args, $atts, $loop_name ) {
		...
		if ( in_array( $atts['type'], apply_filters( 'nc_supported_column_types', array( 'news-grid', 'news-grid-author' ) ) ) ) {
		...
		if ( in_array( $atts['type'], apply_filters( 'nc_supported_posters', array( 'news-poster' ) ) ) ) {
		...
		$class = apply_filters( 'nc_loop_class', $class );
				...
				<?php
					...
					while ( $nc_posts->have_posts() ) : $nc_posts->the_post();
						...
						if ( $newscodes_loop['columns'] !== 1 ) {
							...
							if ( strpos( $atts['type'], 'featured' ) > 0 || in_array( $atts['type'], apply_filters( 'nc_supported_posters', array( 'news-poster' ) ) ) ) {
	...
	public static function multi_factory( $atts, $content = null ) {
		...
		if ( $atts['style'] !== '' ) {
			...
			self::$curr_section['style'] = $atts['style_cs'];
			$nav_class[] = 'newscodes-style-' . $atts['style_cs'];
		}
		else {
	...
	public static function add_image_before_loop( $size = 'full' ) {
		...
		if ( isset( self::$curr_instance['atts']['poster_image'] ) && self::$curr_instance['atts']['poster_image'] !== '' ) {
			$thumb_id = self::get_attachment_id_from_url( self::$curr_instance['atts']['poster_image'] );
			if ( $thumb_id === false ) {
				$set = array(
					esc_url( self::$curr_instance['atts']['poster_image'] ),
					800,
					600
				);
			}
