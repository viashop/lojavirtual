...
class NC_Admin {
	...
	public static function init() {
		...
		self::$generator[] = array(
			'field_id'          => 'nc_poster_image',
			'field_label'       => __( 'Poster Image', 'nwscds' ),
			'field_desc'        => '',
			'field_value'       => '',
			'field_type'        => 'text',
			'field_settings'    => '',
	...
	?>
		<p>
			<?php _e( 'Newscodes styles and settings page.', 'nwscds' ); ?>
		</p>
	<?php
	}
	...
	public static function registration_metabox() {
		$code = get_option( '_newscodes_update_code', '' );
		if ( $code == '' ) {
		?>
			<p class="nc-not-registered nc-box red-box">
				<?php _e( 'Unregistered Version', 'nwscds' ); ?>
			</p>
			<p class="nc-not-registered">
				<?php _e( 'Register Newscodes to get automatic updates! Enter your <a href="http://codecanyon.net" target="_blank">codecanyon.net</a> purchase code.', 'nwscds' ); ?>
			</p>
			<p class="nc-not-registered">
				<input type="text" id="newscodes-purchase-code" name="newscodes-purchase-code" class="newscodes-purchase-code" value="<?php echo $code; ?>" />
			</p>
			<p class="nc-not-registered">
				<span id="nc-purcashe-code" class="button-primary"><?php _e( 'Confirm Code', 'nwscds' ); ?></span>
			</p>
			<?php
		}
		else {
		?>
			<p class="nc-box green-box">
				<?php _e( 'Registered Version', 'nwscds' ); ?>
			</p>
			<p class="nc-registered">
				<span id="nc-purcashe-code-remove" class="button-primary"><?php _e( 'Remove Registration Code', 'nwscds' ); ?></span>
			</p>
		<?php
		}
	}
	...
	public static function ajax_factory() {
		...
		if ( $type == 'new' ) {
			...
			$html .= '<div class="newscodes-multi">';
				...
				$html .= '<nav class="nc-multi-navigation nc-type-' . $atts['type'] . ' newscodes-style">';
