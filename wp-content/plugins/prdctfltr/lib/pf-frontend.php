<?php
	...
	class WC_Prdctfltr {
		...
		function __construct() {
			...
			self::$wc_version = self::version_check();
				...
				if ( self::$wc_version ) {
					...
					if ( isset( $operator ) ) {
						if ( isset( $active['s'] ) ) {
							$pf_tax_query[] = array(
								'relation' => 'AND',
								array( 'taxonomy' => 'product_visibility', 'field' => 'slug', 'terms' => array( 'outofstock' ), 'operator' => $operator ),
								array( 'taxonomy' => 'product_visibility', 'field' => 'slug', 'terms' => array( 'exclude-from-search' ), 'operator' => 'NOT IN' )
							);
						}
						else {
							$pf_tax_query[] = array(
								'relation' => 'AND',
								array( 'taxonomy' => 'product_visibility', 'field' => 'slug', 'terms' => array( 'outofstock' ), 'operator' => $operator ),
								array( 'taxonomy' => 'product_visibility', 'field' => 'slug', 'terms' => array( 'exclude-from-catalog' ), 'operator' => 'NOT IN' )
							);
						}
					}
					else {
						if ( isset( $active['s'] ) ) {
							$pf_tax_query[] = array( 'taxonomy' => 'product_visibility', 'field' => 'slug', 'terms' => array( 'exclude-from-search' ), 'operator' => 'NOT IN' );
						}
						else {
							$pf_tax_query[] = array( 'taxonomy' => 'product_visibility', 'field' => 'slug', 'terms' => array( 'exclude-from-catalog' ), 'operator' => 'NOT IN' );
						}
					}
		...
		function prdctfltr_wc_query( $query ) {
			...
			if ( isset( $pf_activated['orderby'] ) && is_string( $pf_activated['orderby'] ) && $pf_activated['orderby'] !== '' ) {
					...
					if ( !empty( $join ) && !empty( $where ) ) {
						...
						if ( !empty( $variableStockOut ) ) {
							...
							if ( isset( $pf_activated['instock_products'] ) && $pf_activated['instock_products'] == 'out' ) {
								...
								add_filter( 'posts_where' , array( &$this, 'prdctfltr_add_variable_outofstock' ), 99998, 2 );
							}
							else {
		...
		function prdctfltr_wc_tax( $query ) {
			...
			$pf_activated = $prdctfltr_global['active_taxonomies'];;
			$pf_tax_query = self::$settings['tax_query'];
		...
		function prdctfltr_add_variable_outofstock( $where, &$wp_query ) {
...
<?php
		...
		public static function price_to_float( $s ) {
			...
			$s = $s = str_replace( get_woocommerce_currency_symbol(), '', $s );
			$s =  preg_replace( '/\D/', '', $s );

			$decimals = intval( wc_get_price_decimals() );
			if ( $decimals > 0 ) {
				$s = substr( $s, 0, -$decimals );
			}
			...
			switch( $filter ) {
				...
				default :
					if ( is_array( self::$settings['pf_activated'][$filter] ) ) {
						...
						echo ucfirst( self::$settings['pf_activated'][$filter] );
					}
		...
		public static function make_filter() {
			...
			if ( is_shop() || is_product_taxonomy() || is_search() || isset( $wp_query->query_vars['wc_query'] ) && $wp_query->query_vars['wc_query'] == 'product_query' ) {
				...
				self::$settings['ajax_query_vars'] = $wp_query->query_vars;
			}
			else if ( isset( self::$settings['sc_instance'] ) ) {
