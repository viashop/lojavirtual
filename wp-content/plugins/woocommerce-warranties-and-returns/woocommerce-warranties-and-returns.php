...
*/
	...
	class WC_Warranties_And_Returns {
		...
		function __construct() {
			...
			self::$settings['wcwar_default_post'] = get_option( 'wcwar_default_post', 'pending' );
			...
			add_filter( 'woocommerce_add_cart_item_data', array( &$this, 'wc_add_pa_warranty' ), 10, 2 );
			add_filter( 'woocommerce_add_cart_item', array( &$this, 'war_add_product_warranty' ), 10, 1 );

			add_filter('woocommerce_get_cart_item_from_session', array( &$this, 'wc_get_cart_item_from_session' ), 10, 2 );

			add_action( 'woocommerce_add_order_item_meta', array( &$this, 'war_add_warranty_meta' ), 10, 2 );

			add_filter( 'woocommerce_cart_item_price', array( &$this, 'war_cart_price' ), 10, 2 ) ;
		...
		function war_install() {
			...
			if ( !get_option( 'war_settings_page' ) ) {
				...
				$curr_page = array(
					'post_title' => __( 'Request Warranty', 'wcwar'),
		...
		function init_plugin() {
			...
			$curr_args = array(
				...
				'labels' => array(
					'name'              => __( 'Warranty Presets', 'wcwar'),
					'singular_name'     => __( 'Warranty Preset', 'wcwar'),
					'search_items'      => __( 'Search Warranty Presets', 'wcwar'),
					'all_items'         => __( 'All Warranty Presets', 'wcwar'),
					'parent_item'       => __( 'Parent Warranty Preset', 'wcwar'),
					'parent_item_colon' => __( 'Parent Warranty Preset:', 'wcwar'),
					'edit_item'         => __( 'Edit Warranty Preset', 'wcwar'),
					'update_item'       => __( 'Update Warranty Preset', 'wcwar'),
					'add_new_item'      => __( 'Add New Warranty Preset', 'wcwar'),
					'new_item_name'     => __( 'New Warranty Name Preset', 'wcwar')
				),
				'show_ui'               => true,
			...
			$curr_args = array(
				...
				'update_count_callback' => '_update_post_term_count',
				'labels' => array(
					'name'              => __( 'Warranty Status', 'wcwar' ),
					'singular_name'     => __( 'Warranty Status', 'wcwar' ),
					'search_items'      => __( 'Search Warranty', 'wcwar' ),
					'all_items'         => __( 'All Warranty', 'wcwar' ),
					'parent_item'       => __( 'Parent Warranty', 'wcwar' ),
					'parent_item_colon' => __( 'Parent Warranty:', 'wcwar' ),
					'edit_item'         => __( 'Edit Warranty', 'wcwar' ),
					'update_item'       => __( 'Update Warranty', 'wcwar' ),
					'add_new_item'      => __( 'Add New Warranty', 'wcwar' ),
					'new_item_name'     => __( 'New Warranty Name', 'wcwar' )
				),
				'show_ui'               => false,
				'show_in_nav_menus'     => true,
				'query_var'             => true,
			...
			$curr_args = array(
				'label'                 => __( 'Warranties and Returns', 'wcwar' ),
				'labels'                => array(
					'name'              => __( 'Warranty and Returns Requests', 'wcwar' ),
					'singular_name'     => __( 'Warranty and Return Request', 'wcwar' ),
					'all_items'         => __( 'All Requests', 'wcwar' ),
					'menu_name'         => __( 'Warranties and Returns', 'wcwar' ),
					'not_found'         => __( 'No requests found', 'wcwar' ),
					'edit_item'         => __( 'Edit Request', 'wcwar' ),
					'add_new_item'      => __( 'Create a Request', 'wcwar' ),
					'new_item_name'     => __( 'Create a Request', 'wcwar' ),
					'parent_item_colon' => __( 'Parent Request', 'wcwar' ),
					'view_item'         => __( 'View Request', 'wcwar' ),
					'search_items'      => __( 'Search Requests', 'wcwar' )
				),
				'public'                => true,
			...
			if ( empty( $curr_status) ) {
				...
				$curr_warranty_status = array(
					'new' => __( 'New', 'wc_warranty' ),
					'processing' => __( 'Processing', 'wc_warranty' ),
					'completed' => __( 'Completed', 'wc_warranty' ),
					'rejected' => __( 'Rejected', 'wc_warranty' )
				);
		...
		function wc_add_product_tab( $product_data_tabs ) {
			...
			//echo ' <li class="wcwar_tab hide_if_external hide_if_variable"><a href="#wcwar_tab">'. __( 'Warranties and Returns', 'wcwar') .'</a></li>';
		}
				...
				var wcwar_metabox = "<div class=\"wcwar_metabox\">\
					...
					<div class=\"options_group grouping\" data-group=\"included_warranty\">\
						<p class=\"form-field wcwar_qp_price\">\
							<label for=\"wcwar_qp_price\">' . __( 'Additional warranty price', 'wcwar') . '</label>\
							<input type=\"number\" class=\"option short wc_input_price\" name=\"wcwar_qp_price_single[]\" value=\"\" placeholder=\"\" step=\"any\">\
							<em>' . __( 'Enter additional price for this warranty.', 'wcwar' ) . '</em>\
						</p>\
						<p class=\"form-field wcwar_qp_type\">\
							<label for=\"wcwar_qp_type\">' . __( 'Warranty period type', 'wcwar') . '</label>\
							<select name=\"wcwar_qp_type_single[]\" class=\"option select short\">\
								<option value=\"\">' . __( 'Not selected', 'wcwar') . '</option>\
								<option value=\"days\">' . __( 'Days', 'wcwar') . '</option>\
								<option value=\"weeks\">' . __( 'Weeks', 'wcwar') . '</option>\
								<option value=\"months\">' . __( 'Months', 'wcwar') . '</option>\
								<option value=\"years\">' . __( 'Years', 'wcwar') . '</option>\
								<option value=\"lifetime\">' . __( 'Lifetime', 'wcwar') . '</option>\
							</select>\
							<em>' . __( 'Select warranty period type.', 'wcwar' ) . '</em>\
						</p>\
						<p class=\"form-field wcwar_qp_period\">\
							<label for=\"wcwar_qp_period\">' . __( 'Warranty period', 'wcwar') . '</label>\
							<input type=\"number\" class=\"option short\" name=\"wcwar_qp_period_single[]\" value=\"\" placeholder=\"\" step=\"any\">\
							<em>' . __( 'Enter warranty period.', 'wcwar' ) . '</em>\
						</p>\
						<p class=\"form-field wcwar_qp_desc\">\
							<label for=\"wcwar_qp_desc\">' . __( 'Warranty description (optional)', 'wcwar') . '</label>\
							<textarea class=\"option short\" name=\"wcwar_qp_desc_single[]\" placeholder=\"\" step=\"any\"></textarea>\
							<em>' . __( 'Enter warranty description.', 'wcwar' ) . '</em>\
						</p>\
						<p class=\"form-field wcwar_qp_thumb\">\
							<span class=\"thumb_preview\"></span>\
							<label for=\"wcwar_qp_thumb\">' . __( 'Warranty thumbnail (optional)', 'wcwar') . '</label>\
							<input type=\"hidden\" class=\"option short\" name=\"wcwar_qp_thumb_single[]\" value=\"\" placeholder=\"\" step=\"any\">\
							<button type=\"button\" class=\"option button add_wcwar_qp_thumb\">' . __( 'Add warranty thumbnail', 'wcwar') . '</button>\
						</p>\
					</div>\
				</div>";
		...
		<div id="wcwar_tab" class="panel woocommerce_options_panel">
			...
			<div class="options_group grouping basic">
				<p class="form-field wcwar_type">
					<label for="wcwar_type"><?php _e( 'Select warranty type', 'wcwar' ); ?></label>
					<select id="wcwar_type" name="wcwar_type_single" class="option select short">
						<option value="no_warranty" <?php if ( $curr_warranty['type'] == 'no_warranty') echo 'selected'; ?>><?php _e( 'No warranty', 'wcwar' ); ?></option>
						<option value="preset_warranty" <?php if ( $curr_warranty['type'] == 'preset_warranty') echo 'selected'; ?>><?php _e( 'Preset warranty', 'wcwar' ); ?></option>
						<option value="quick_warranty" <?php if ( $curr_warranty['type'] == 'quick_warranty') echo 'selected'; ?>><?php _e( 'Quick warranty', 'wcwar' ); ?></option>
					</select>
					<em><?php _e( 'Select preset warranty or add quick product warranty.', 'wcwar' ); ?></em>
				</p>
			</div>
			...
			<div class="options_group grouping preset" data-group="preset_warranty"<?php echo !empty( $curr_disable ) ? ' style="display:none;"' : ''; ?>>
				<p class="form-field wcwar_preset">
					<label for="wcwar_preset"><?php _e( 'Select warranty preset', 'wcwar' ); ?></label>
					<select id="wcwar_preset" name="wcwar_preset_single" class="option select short"<?php echo $curr_disable; ?>>
						<option value="" <?php if (isset( $curr_warranty['preset'] ) && $curr_warranty['preset'] == '') echo 'selected'; ?>><?php _e( 'Not selected', 'wcwar' ); ?></option>
					<?php
					...
					<em><?php _e( 'Select warranty preset to use with the current product. Warranty presets can be set in Products &gt; Warranty Presets.', 'wcwar' ); ?></em>
				</p>
			</div>
			...
			<div class="options_group grouping type" data-group="quick_warranty"<?php echo !empty( $curr_disable ) ? ' style="display:none;"' : ''; ?>>
				<p class="form-field wcwar_q_type">
					<label for="wcwar_q_type"><?php _e( 'Select quick warranty type', 'wcwar' ); ?></label>
					<select id="wcwar_q_type" name="wcwar_q_type_single" class="option select short"<?php echo $curr_disable; ?>>
						<option value=""><?php _e( 'Not selected', 'wcwar' ); ?></option>
						<option value="included_warranty" <?php if (isset( $curr_warranty['quick'] ) && $curr_warranty['quick'] == 'included_warranty') echo 'selected'; ?>><?php _e( 'Included warranty', 'wcwar' ); ?></option>
						<option value="paid_warranty" <?php if (isset( $curr_warranty['quick'] ) && $curr_warranty['quick'] == 'paid_warranty') echo 'selected'; ?>><?php _e( 'Paid warranty', 'wcwar' ); ?></option>
					</select>
					<em><?php _e( 'Warranties can be included or paid as an add-on. Choose your warranty type.', 'wcwar' ); ?></em>
				</p>
			</div>
			...
			<div class="options_group grouping" data-group="included_warranty"<?php echo !empty( $curr_disable ) ? ' style="display:none;"' : ''; ?>>
				<p class="form-field wcwar_qi_type">
					<label for="wcwar_qi_type"><?php _e( 'Warranty period type', 'wcwar' ); ?></label>
					<select id="wcwar_qi_type" name="wcwar_qi_type_single" class="option select short"<?php echo $curr_disable; ?>>
						...
						?>
						<option value=""><?php _e( 'Not selected', 'wcwar' ); ?></option>
						<option value="days" <?php if (isset( $curr_options['type'] ) && $curr_options['type'] == 'days') echo 'selected'; ?>><?php _e( 'Days', 'wcwar' ); ?></option>
						<option value="weeks" <?php if (isset( $curr_options['type'] ) && $curr_options['type'] == 'weeks') echo 'selected'; ?>><?php _e( 'Weeks', 'wcwar' ); ?></option>
						<option value="months" <?php if (isset( $curr_options['type'] ) && $curr_options['type'] == 'months') echo 'selected'; ?>><?php _e( 'Months', 'wcwar' ); ?></option>
						<option value="years" <?php if (isset( $curr_options['type'] ) && $curr_options['type'] == 'years') echo 'selected'; ?>><?php _e( 'Years', 'wcwar' ); ?></option>
						<option value="lifetime" <?php if (isset( $curr_options['type'] ) && $curr_options['type'] == 'lifetime') echo 'selected'; ?>><?php _e( 'Lifetime', 'wcwar' ); ?></option>
					</select>
					<em><?php _e( 'Select warranty period type.', 'wcwar' ); ?></em>
				</p>
				<p class="form-field wcwar_qi_period">
					<label for="wcwar_qi_period"><?php _e( 'Warranty period', 'wcwar' ); ?></label>
					...
					<em><?php _e( 'Enter warranty period.', 'wcwar' ); ?></em>
				</p>
				<p class="form-field wcwar_qi_desc">
					<label for="wcwar_qi_desc"><?php _e( 'Warranty description (optional)', 'wcwar' ); ?></label>
					<textarea class="option short" name="wcwar_qi_desc_single" id="wcwar_qi_desc" placeholder="" step="any"<?php echo $curr_disable; ?>><?php if (isset( $curr_options['desc'] ) && $curr_options['desc'] !== '') echo $curr_options['desc']; ?></textarea>
					<em><?php _e( 'Enter warranty description.', 'wcwar' ); ?></em>
				</p>
				<p class="form-field wcwar_qi_thumb">
					<span class="thumb_preview"><?php if (isset( $curr_options['thumb'] ) && $curr_options['thumb'] !== '') echo '<img width="64" height="auto" src="' . $curr_options['thumb'] . '" alt="' .__( 'Thumbnail preview', 'wcwar') . '" />'; ?></span>
					<label for="wcwar_qi_thumb"><?php _e( 'Warranty thumbnail (optional)', 'wcwar' ); ?></label>
					...
					<button type="button" class="option button add_wcwar_qi_thumb"<?php echo $curr_disable; ?>><?php _e( 'Add warranty thumbnail', 'wcwar' ); ?></button>
				</p>
			</div>
			...
			<div class="options_group grouping" data-group="paid_warranty"<?php echo !empty( $curr_disable ) ? ' style="display:none;"' : ''; ?>>
				<p class="form-field wcwar_qp_without_single">
					<label for="wcwar_qp_without_single"><?php _e( 'Add <em>No Warranty</em> option', 'wcwar' ); ?></label>
					...
					<em><?php _e( 'To enable product purchases without a paid warranty check this option.', 'wcwar' ); ?></em>
				</p>
			</div>
			...
			<div class="options_group grouping" data-group="paid_warranty"<?php echo !empty( $curr_disable ) ? ' style="display:none;"' : ''; ?>>
				...
				<?php
					...
					?>
						<div class="wcwar_metabox">
							...
							<div class="options_group grouping" data-group="included_warranty">
								<p class="form-field wcwar_qp_price">
									<label for="wcwar_qp_price"><?php _e( 'Additional warranty price', 'wcwar' ); ?></label>
									...
									<em><?php _e( 'Enter additional price for this warranty.', 'wcwar' ); ?></em>
								</p>
								<p class="form-field wcwar_qp_type">
									<label for="wcwar_qp_type"><?php _e( 'Warranty period type', 'wcwar' ); ?></label>
									<select name="wcwar_qp_type_single[]" class="option select short">
										<option value=""><?php _e( 'Not selected', 'wcwar' ); ?></option>
										<option value="days" <?php if (isset( $warranty['type'] ) && $warranty['type'] == 'days') echo 'selected'; ?>><?php _e( 'Days', 'wcwar' ); ?></option>
										<option value="weeks" <?php if (isset( $warranty['type'] ) && $warranty['type'] == 'weeks') echo 'selected'; ?>><?php _e( 'Weeks', 'wcwar' ); ?></option>
										<option value="months" <?php if (isset( $warranty['type'] ) && $warranty['type'] == 'months') echo 'selected'; ?>><?php _e( 'Months', 'wcwar' ); ?></option>
										<option value="years" <?php if (isset( $warranty['type'] ) && $warranty['type'] == 'years') echo 'selected'; ?>><?php _e( 'Years', 'wcwar' ); ?></option>
										<option value="lifetime" <?php if (isset( $warranty['type'] ) && $warranty['type'] == 'lifetime') echo 'selected'; ?>><?php _e( 'Lifetime', 'wcwar' ); ?></option>
									</select>
									<em><?php _e( 'Select warranty period type.', 'wcwar' ); ?></em>
								</p>
								<p class="form-field wcwar_qp_period">
									<label for="wcwar_qp_period"><?php _e( 'Warranty period', 'wcwar' ); ?></label>
								...
								<p class="form-field wcwar_qp_desc">
									<label for="wcwar_qp_desc"><?php _e( 'Warranty description (optional)', 'wcwar' ); ?></label>
								...
								<p class="form-field wcwar_qp_thumb">
									<span class="thumb_preview"><?php if (isset( $warranty['thumb'] ) && $warranty['thumb'] !== '') echo '<img width="64" height="auto" src="' . $warranty['thumb'] . '" alt="' . __( 'Thumbnail preview', 'wcwar') . '" />'; ?></span>
									<label for="wcwar_qp_thumb"><?php _e( 'Warranty thumbnail (optional)', 'wcwar' ); ?></label>
									...
									<button type="button" class="option button add_wcwar_qp_thumb"><?php _e( 'Add warranty thumbnail', 'wcwar' ); ?></button>
								</p>
							</div>
						</div>
					<?php
				...
				<p class="toolbar">
					<button type="button" class="option button button-primary add_paid_warranty"<?php echo $curr_disable; ?>><?php _e( 'Add paid warranty', 'wcwar' ); ?></button>
				</p>
			</div>
		...
		function wc_add_variable_tab_24( $loop, $data, $variation ) {
			...
			$curr_warranty = is_array( $curr_warranty ) ? maybe_unserialize( $curr_warranty[0] ) : array();
		...
		?>
			...
			<?php
				woocommerce_wp_select(
					array(
						'id'          => 'wcwar_type[' . $loop . ']',
						'label'       => __( 'Select warranty type', 'wcwar' ),
						'description' => __( 'Select preset warranty or add quick product warranty.', 'wcwar' ),
						'value'       => $curr_warranty['type'],
						'class'       => 'wcwar_type option select short',
						'wrapper_class' => 'form-row form-row-first',
						'options' => array(
							'no_warranty'   => __( 'No warranty', 'wcwar' ),
							'preset_warranty'   => __( 'Preset warranty', 'wcwar' ),
							'quick_warranty' => __( 'Quick warranty', 'wcwar' )
						)
					)
				);
			?>
			...
			<?php
				woocommerce_wp_select(
					array(
						...
						'label'       => __( 'Select warranty preset', 'wcwar' ),
						'description' => __( 'Select warranty preset to use with the current product. Warranty presets can be set in Products &gt; Warranty Presets.', 'wcwar' ),
			...
			<?php
				woocommerce_wp_select(
					array(
						...
						'label'       => __( 'Select quick warranty type', 'wcwar' ),
						'description' => __( 'Warranties can be included or paid as an add-on. Choose your warranty type.', 'wcwar' ),
						...
						'options' => array(
							'' => __( 'Not selected', 'wcwar' ),
							'included_warranty' => __( 'Included warranty', 'wcwar' ),
							'paid_warranty' => __( 'Paid warranty', 'wcwar' )
						)
					)
				);
			?>
			...
			<?php
				woocommerce_wp_select(
					array(
						...
						'label'       => __( 'Warranty period type', 'wcwar' ),
						'description' => __( 'Select warranty period type.', 'wcwar' ),
						...
						'options' => array(
							'' => __( 'Not selected', 'wcwar' ),
							'days' => __( 'Days', 'wcwar' ),
							'weeks' => __( 'Weeks', 'wcwar' ),
							'months' => __( 'Months', 'wcwar' ),
							'years' => __( 'Years', 'wcwar' ),
							'lifetime' => __( 'Lifetime', 'wcwar' )
						)
					)
				);
				...
				woocommerce_wp_text_input(
					array( 
						...
						'label'       => __( 'Warranty period', 'wcwar' ),
						'description' => __( 'Enter warranty period.', 'wcwar' ),
				...
				woocommerce_wp_textarea_input(
					array( 
						...
						'label'       => __( 'Warranty description (optional)', 'wcwar' ),
						'placeholder' => '',
						'description' => __( 'Enter warranty description.', 'wcwar' ),
			...
			?>
				<span class="thumb_preview"><?php echo isset( $curr_options['thumb'] ) && $curr_options['thumb'] !== '' ? '<img width="64" height="auto" src="' . $curr_options['thumb'] . '" alt="' . esc_attr( __( 'Thumbnail preview', 'wcwar' ) ) . '" />' : '<img width="64" height="auto" src="' . self::$url_path . 'assets/images/no_image.gif" alt="' . esc_attr( __( 'No image', 'wcwar' ) ) . '" />'; ?></span>
				<button type="button" class="option button add_wcwar_qi_thumb"<?php echo !empty( $curr_disable ) ? ' disabled="disabled"' : ''; ?>><?php _e( 'Add warranty thumbnail', 'wcwar' ); ?></button>
			<?php

				woocommerce_wp_hidden_input(
					array(
						'id'          => 'wcwar_qi_thumb[' .  $loop . ']',
						'label'       => __( 'Warranty thumbnail (optional)', 'wcwar' ),
						'placeholder' => '',
						'description' => __( 'Add a warranty thumbnail.', 'wcwar' ),
			...
			?>
				...
				<?php
					woocommerce_wp_checkbox(
						array(
							...
							'label'         => __( 'Add <em>No Warranty</em> option', 'wcwar' ),
							'description'   => __( 'To enable product purchases without a paid warranty check this option.', 'wcwar' ),
				...
				?>
					...
					<p class="toolbar">
						<button type="button" class="option button button-primary add_paid_warranty"<?php echo !empty( $curr_disable ) ? ' disabled="disabled"' : ''; ?>><?php _e( 'Add paid warranty', 'wcwar' ); ?></button>
					</p>
				</div>
			</div>
		<?php
		...
		?>
			<tr class="panel woocommerce_variation_panel wcwar_tab">
				...
				<td class="options_group grouping basic">
					<h3><?php _e( 'Item Warranties', 'wcwar' ); ?></h3>
					<p class="form-field wcwar_type">
						<label for="wcwar_type"><?php _e( 'Select warranty type', 'wcwar' ); ?></label>
				...
				<td class="options_group grouping" data-group="paid_warranty"<?php echo !empty( $curr_disable ) ? ' style="display:none;"' : ''; ?>>
					<p class="form-field wcwar_qp_without">
						<label for="wcwar_qp_without"><?php _e( 'Add <em>No Warranty</em> option', 'wcwar' ); ?></label>
	...
	?>
				...
				var wcwar_metabox = "<div class=\"wcwar_metabox\">\
							...
							<div class=\"options_group grouping\" data-group=\"included_warranty\">\
								...
								<p class=\"form-field wcwar_qp_type\">\
									...
									<select name=\"wcwar_qp_type[]\" class=\"option select short\">\
										<option value=\"\"><?php  _e( 'Not selected', 'wcwar' ); ?></option>\
										<option value=\"days\"><?php  _e( 'Days', 'wcwar' ); ?></option>\
										<option value=\"weeks\"><?php  _e( 'Weeks', 'wcwar' ); ?></option>\
										<option value=\"months\"><?php  _e( 'Months', 'wcwar' ); ?></option>\
										<option value=\"years\"><?php  _e( 'Years', 'wcwar' ); ?></option>\
										<option value=\"lifetime\"><?php  _e( 'Lifetime', 'wcwar' ); ?></option>\
									</select>\
									<em><?php _e( 'Select warranty period type.', 'wcwar' ); ?></em>\
								</p>\
								<p class=\"form-field wcwar_qp_period\">\
								...
								<p class=\"form-field wcwar_qp_desc\">\
									<label for=\"wcwar_qp_desc\"><?php  _e( 'Warranty description (optional)', 'wcwar' ); ?></label>\
									<textarea class=\"option short\" name=\"wcwar_qp_desc[]\" placeholder=\"\" step=\"any\"></textarea>\
									<em><?php _e( 'Enter warranty descriptio.', 'wcwar' ); ?></em>\
								</p>\
								<p class=\"form-field wcwar_qp_thumb\">\
									<span class=\"thumb_preview\"></span>\
									<label for=\"wcwar_qp_thumb\"><?php  _e( 'Warranty thumbnail (optional)', 'wcwar' ); ?></label>\
		...
		<div id="wcwar_tab" class="form-field">
			...
			<div class="options_group grouping type" data-group="quick_warranty">
				<p class="form-field wcwar_q_type">
					<label for="wcwar_q_type"><?php _e( 'Select warranty type', 'wcwar' ); ?></label>
	...
	?>
		...
		<tr class="form-field wcwar-field">
			<th scope="row" valign="top"><label><?php _e( 'Warranty', 'wcwar' ); ?></label></th>
			<td>
				<div id="wcwar_tab" class="form-field">
...
/*			if ( $posts = $columns['description'] ){ unset( $columns['description'] ); }*/
		...
		function war_admin_scripts( $hook ) {
			...
			if ( $hook == 'post.php' || $hook == 'edit.php' || $hook == 'post-new.php' ) {
				...
				$localize = array(
					...
					'localization' => array(
						'delete' => __( 'Delete?', 'wcwar' ),
						'notselected' => __( 'Not selected.', 'wcwar' ),
						'deleted' => __( 'Deleted!', 'wcwar' ),
						'error' => __( 'Error!', 'wcwar' ),
						'load' => __( 'Load?', 'wcwar' ),
						'loaded' => __( 'Loaded!', 'wcwar' ),
						'templatename' => __( 'Template name?', 'wcwar' ),
						'missing' => __( 'Missing email or name.', 'wcwar' ),
						'sendemail' => __( 'Send Email?', 'wcwar' ),
						'saved' => __( 'Saved!', 'wcwar' ),
						'emailsent' => __( 'Email sent!', 'wcwar' ),
					)
				);
			...
			foreach ( $ids as $id ) {
				...
				?>
					<div class="war_warranty<?php echo $curr_class; ?>" data-id="<?php echo $id; ?>">
						<<?php echo $title_tag; ?>><?php _e( 'Included Warranty', 'wcwar' ); ?></<?php echo $title_tag; ?>>
						<p>
				...
				?>
					<div class="war_warranty war_paid<?php echo $curr_class; ?>" data-id="<?php echo $id; ?>">
						<<?php echo $title_tag; ?>><?php _e( 'Select Warranty Options', 'wcwar' ); ?></<?php echo $title_tag; ?>>
						<p>
							<select name="wcwar_pa_warranty">
								<?php if ( isset( $curr_warranty['paid_no_warranty'] ) && $curr_warranty['paid_no_warranty'] == 'yes' ) { ?>
									<option value="no_warranty"><?php _e( 'No warranty' ); ?></option>
								<?php } ?>
						...
						<p class="war_option" data-selected="no_warranty">
							<strong><?php _e( 'Not selected', 'wcwar' ); ?></strong><br/>
						<small><?php _e( 'Purchase this product without any warranty options.', 'wcwar' ); ?></small>
						</p>
						<?php } ?>
						<?php for ( $i = 0; $i < count( $curr_warranty['paid_warranty'] ); $i++ ) { ?>
		...
		function war_add_product_warranty( $product_data ) {
			global $woocommerce;
			...
			$product_id = method_exists( $curr_product, 'get_id' ) ? absint( $curr_product->get_id() ) : absint( $product->variation_id );
...
/*		function war_add_cart_prices( $cart_object ) {
			$custom_price = 10;
		...
		function wc_get_cart_item_from_session( $product_data, $product_values) {
			if ( isset( $product_values['wcwar_pa_warranty'] ) ) {
				$product_data['wcwar_pa_warranty'] = $product_values['wcwar_pa_warranty'];

				$product_id = ( isset( $product_data['variation_id'] ) && $product_data['variation_id'] !== '' ? $product_data['variation_id'] : $product_data['product_id'] );
...
/*				$curr_warranty = get_post_meta( $product_id, '_wcwar_warranty', true );
				...
				if ( $curr_warranty ) {
					...
					if ( isset( $curr_warranty['quick'] ) && $curr_warranty['quick'] !== '' ) {
						if ( $curr_warranty['quick'] == 'paid_warranty' && isset( $curr_warranty['paid_warranty'] ) ) {
							if ( $product_data['wcwar_pa_warranty'] == '' || $product_data['wcwar_pa_warranty'] == 'no_warranty' ) {
								
							}
							else if ( $product_data['wcwar_pa_warranty'] == '' || $product_data['wcwar_pa_warranty'] == 'not_selected' ) {
								if ( isset( $curr_warranty['paid_no_warranty'] ) && $curr_warranty['paid_no_warranty'] == 'yes' ) {
...
*/
				...
				$product_data = self::war_add_product_warranty( $product_data);
			}
			return $product_data;
		}
		...
		function war_order( $order ) {
			...
			if ( sizeof( $order->get_items() ) > 0 ) {
				...
				<h2><?php _e( 'Available Warranties for this Order', 'wcwar' ); ?></h2>
				<?php
					if ( !in_array( $order->get_status(), apply_filters( 'wcwar_warranty_order_status', array( 'completed' ), $order ) ) ) {
				?>
				<p class="wcwar_status blue">
					<span class="wcwar_info_icon"><i class="wcwar-info"></i></span> <?php _e( 'Your warranties will be available once your order is complete.', 'wcwar' ); ?>
				</p>
				<?php
					$curr_complete = get_post_meta( $id, '_ordered_date', true );
					$warranty_status_hold[] = '<p class="wcwar_status blue"><span class="wcwar_info_icon"><i class="wcwar-info"></i></span>' . __( 'Pending order', 'wcwar') . '</p>';
				...
				<table class="shop_table wcwarranty">
					<thead>
						<tr>
							<th><?php _e( 'Product', 'wcwar' ); ?></th>
							<th><?php _e( 'Warranty', 'wcwar' ); ?></th>
							<th><?php _e( 'Status', 'wcwar' ); ?></th>
							<th><?php _e( 'Action', 'wcwar' ); ?></th>
						</tr>
					</thead>
					<tbody>
				<?php
							...
							foreach ( $curr_req as $req ) {
								...
								if ( $return !== '' && $return == 'return' ) {
									if ( !empty ( $curr_terms) && $curr_terms->slug == 'completed' ) {
										$curr_addon .= '<span class="wcwar_status grey"><span class="wcwar_info_icon"><i class="wcwar-cross"></i></span><span>' . __( 'This item has been returned to the store.', 'wcwar' ) . '</span></span>';
										$curr_link[] = '';
									}
									else if ( $req->post_status == 'pending' ) {
										$curr_addon .= '<span class="wcwar_status blue"><span class="wcwar_info_icon"><i class="wcwar-info"></i></span><span>' . __( 'Pending review', 'wcwar' ) . '</span></span>';
										$curr_link[] = '';
									}
									else if ( !empty ( $curr_terms) && $curr_terms->slug == 'rejected' ) {
										$curr_addon .= '<span class="wcwar_status grey"><span class="wcwar_info_icon"><i class="wcwar-cross"></i></span><span>' . __( 'Return rejected', 'wcwar' ) . '</span></span>';
										$curr_link[] = '<a href="' . get_permalink( $curr_reqId ) . '" class="wcwar_request_button button">' . __( 'View Request', 'wcwar' ) . '</a>';
									}
									else {
										$curr_addon .= '<span class="wcwar_status yellow"><span class="wcwar_info_icon"><i class="wcwar-info"></i></span><span>' . __( 'Return requested', 'wcwar' ) . '</span></span>';
										$curr_link[] = '<a href="' . get_permalink( $curr_reqId ) . '" class="wcwar_request_button button">' . __( 'View Request', 'wcwar' ) . '</a>';
									}
									$requested = true;
									...
									if ( !empty ( $curr_terms) && $curr_terms->slug == 'completed' ) {
										$curr_addon .= '<span class="wcwar_status blue"><span class="wcwar_info_icon"><i class="wcwar-check"></i></span><span>' . __( 'Completed', 'wcwar' ) . $curr_numid . '</span></span>';
										$curr_link[] = '<a href="' . get_permalink( $curr_reqId ) . '" class="wcwar_request_button button">' . __( 'View Request', 'wcwar' ) . $curr_numid . '</a>';
									}
									else if( !empty ( $curr_terms) && $curr_terms->slug == 'rejected' ) {
										$curr_addon .= '<span class="wcwar_status grey"><span class="wcwar_info_icon"><i class="wcwar-cross"></i></span><span>' . __( 'Warranty rejected', 'wcwar' ) . '</span></span>';
										$curr_link[] = '<a href="' . get_permalink( $curr_reqId ) . '" class="wcwar_request_button button">' . __( 'View Request', 'wcwar' ) . $curr_numid . '</a>';
									}
									else if ( !empty ( $curr_terms) && $req->post_status !== 'pending' ) {
										$curr_addon .= '<span class="wcwar_status yellow"><span class="wcwar_info_icon"><i class="wcwar-info"></i></span><span>' . __( 'Requested', 'wcwar' ) . '</span></span>';
										$curr_link[] = '<a href="' . get_permalink( $curr_reqId ) . '" class="wcwar_request_button button">' . __( 'View Request', 'wcwar' ) . $curr_numid . '</a>';
										$requested = true;
									}
									else {
	...
	/*						if ( $requested === false && $curr_multi_req == 'yes' ) {
								$warranty_status[] = '<span class="wcwar_status green"><span class="wcwar_info_icon"><i class="wcwar-check"></i></span><span>' . __( 'Valid', 'wcwar' ) . '</span></span>';
								$curr_link[] = '<a href="' . $curr_request . '" class="wcwar_request_button button">' . __( 'Request Warranty', 'wcwar' ) . '</a>';
							}*/
						}

						if ( $curr_notallowed === false && $curr_status === 'nowar' && $curr_returns == 'yes' /*&& empty( $curr_req )*/ ) {

							$addon_msg = __( 'This item was sold without warranty', 'wcwar' );

							if ( self::hlp_valid_return( $curr_complete, $item) === true ) {
								$addon_msg .= __( ', however you can still return it to the store', 'wcwar' );
								$warranty_status[] = '<p class="wcwar_status green"><span class="wcwar_info_icon"><i class="wcwar-check"></i></span>' . __( 'Return available', 'wcwar') . '</p>';
								$curr_link[] = '<a href="' . $curr_request . '" class="wcwar_request_button button">' . __( 'Request Return', 'wcwar') . '</a>';
							}
							else {
								$warranty_status[] = '<p class="wcwar_status grey"><span class="wcwar_info_icon"><i class="wcwar-cross"></i></span>' . __( 'No warranty', 'wcwar') . '</p>';
							}
							...
							$addon_msg = __( 'Warranty for this item has expired', 'wcwar' );
							$warranty_status[] = '<p class="wcwar_status grey"><span class="wcwar_info_icon"><i class="wcwar-cross"></i></span>' . __( 'Expired', 'wcwar') . '</p>';
							$request_button = false;
						}
						...
						if ( isset( $item['wcwar_warranty'] ) ) {
							...
							?>
										<span class="wcwar_block_wrap">
											...
											<span class="wcwar_warranty_period">
												<span>
													<?php echo __( 'Included Warranty', 'wcwar' ) . ' - ' . $curr_warranty['included_warranty']['period'] . ' ' . self::hlp_get_warranty_string( $curr_warranty['included_warranty']['period'], $curr_warranty['included_warranty']['type'] ); ?>
												</span>
												<?php
										...
										<?php
											...
											/*
												...
												$warranty_status = '<span class="wcwar_status blue"><span class="wcwar_info_icon"><i class="wcwar-check"></i></span><span>' . __( 'Completed', 'wcwar' ) . '</span></span>';
											}
											else {
												$warranty_status = '<span class="wcwar_status blue"><span class="wcwar_info_icon"><i class="wcwar-info"></i></span><span>' . __( 'Pending Order', 'wcwar' ) . '</span></span>';
											}*/
								}
								else if ( $curr_warranty['quick'] == 'paid_warranty' && isset( $curr_warranty['paid_warranty'] ) ) {
							...
							?>
										<span class="wcwar_block_wrap">
											...
											<span class="wcwar_warranty_period">
												<span>
													<?php echo __( 'Paid Warranty', 'wcwar') . ' (+' . wc_price( $curr_warranty['paid_warranty'][$curr]['price'] ) . ')' . ' - ' . $curr_warranty['paid_warranty'][$curr]['period'] . ' ' . self::hlp_get_warranty_string( $curr_warranty['paid_warranty'][$curr]['period'], $curr_warranty['paid_warranty'][$curr]['type'] ); ?>
												</span>
												<?php
										...
										_e( 'This item was sold without warranty', 'wcwar' );
									}
								}
							}
							else {
						...
						?>
							...
							<?php
								if ( $curr_notallowed === false && $request_button === true && !isset( $curr_link ) ) {
									echo '<a href="' . $curr_request . '" class="wcwar_request_button button">' . __( 'Request Warranty', 'wcwar') . '</a>';
								}
								else {
		...
		function war_add_warranty_meta( $item_id, $values ) {

			$curr_product= $values['data'];

			$product_id = ( isset( $curr_product->variation_id ) ) ? $curr_product->variation_id : $curr_product->id;
		...
		function war_items_warranty_column_header() {
			echo '<th class="wcwar_warranty_item">' . __( 'Warranty Type', 'wcwar') . '</th>';
			echo '<th class="wcwar_warranty_status">' . __( 'Warranty Status', 'wcwar') . '</th>';
		}
		...
		function war_items_warranty_column( $curr_product, $curr_item, $curr_item_id ) {
			...
			if ( $curr_order !== false ) {
				...
				if ( $status && $status === 'nowar' ) {
					$curr_fields['warranty'] = __( 'No Warranty', 'wcwar' );
					$curr_fields['status'] = '<span class="wcwar_badge warranty_nowar">' . __( 'No Warranty', 'wcwar') . '</span>';
				}
				else if ( $status ) {
					$curr_fields['status'] = '<span class="wcwar_badge warranty_rejected">' . __( 'Warranty Expired', 'wcwar') . '</span>';
				}
				...
				if ( !empty( $curr_req) ) {
					...
					if ( !empty ( $curr_terms) ) {
						...
						if ( $return !== '' && $return == 'return' ) {
							$curr_fields['status'] = '<span class="wcwar_badge warranty_only">' . __( 'Return', 'wcwar') . '</span>';
						}
						else {
							$curr_fields['status'] = '<span class="wcwar_badge return_only">' . __( 'Warranty', 'wcwar') . '</span>';
						}

						$switch_slug = $curr_terms->slug;
						if ( $switch_slug == 'new' ) {
							$curr_fields['status'] .= '<a href="'.$curr_req[0]->guid.'" class="wcwar_badge wcwar_change warranty_new" title="' . __( 'View Request', 'wcwar') . '">' . __( 'New', 'wcwar') . '</a>';
							$curr_fields['status'] .= '<a href="' . get_edit_post_link( $curr_req[0]->ID ) . '" class="wcwar_badge warranty_button view_warranty" title="' . __( 'View Request', 'wcwar') . '"><i class="wcwar-view"></i></a>';
						}
						else if ( $switch_slug == 'processing' ) {
							$curr_fields['status'] .= '<a href="'.$curr_req[0]->guid.'" class="wcwar_badge wcwar_change warranty_processing" title="' . __( 'View Request', 'wcwar') . '">' . __( 'Processing', 'wcwar') . '</a>';
							$curr_fields['status'] .= '<a href="' . get_edit_post_link( $curr_req[0]->ID ) . '" class="wcwar_badge warranty_button view_warranty" title="' . __( 'View Request', 'wcwar') . '"><i class="wcwar-view"></i></a>';
						}
						else if ( $switch_slug == 'completed' ) {
							$curr_fields['status'] .= '<a href="'.$curr_req[0]->guid.'" class="wcwar_badge wcwar_change warranty_completed" title="' . __( 'View Request', 'wcwar') . '">' . __( 'Completed', 'wcwar') . '</a>';
							$curr_fields['status'] .= '<a href="' . get_edit_post_link( $curr_req[0]->ID ) . '" class="wcwar_badge warranty_button view_warranty" title="' . __( 'View Request', 'wcwar') . '"><i class="wcwar-view"></i></a>';
						}
						else if ( $switch_slug == 'rejected' ) {
							$curr_fields['status'] .= '<a href="'.$curr_req[0]->guid.'" class="wcwar_badge wcwar_change warranty_rejected" title="' . __( 'View Request', 'wcwar') . '">' . __( 'Rejected', 'wcwar') . '</a>';
							$curr_fields['status'] .= '<a href="' . get_edit_post_link( $curr_req[0]->ID ) . '" class="wcwar_badge warranty_button view_warranty" title="' . __( 'View Request', 'wcwar') . '"><i class="wcwar-view"></i></a>';
						}
					}
					$curr_fields['status'] .= '<a href="#" class="wcwar_badge warranty_button warranty_change_status" data-id="' . $curr_req[0]->ID . '" title="' . __( 'Change Request Status', 'wcwar') . '"><i class="wcwar-change"></i></a>';
				}
				...
				$admin_url = admin_url( 'post-new.php?post_type=wcwar_warranty_req&order_id=' . $curr_order . $curr_parent . '&item_id=' . $curr_item_id ) . '&post_title=' . __( 'Request for Order', 'wcwar' ) . ' %23' . $curr_order . ' - ' . __( 'Item', 'wcwar') . ' %23' . $curr_item_id;

				if ( isset( $curr_warranty['quick'] ) && $curr_warranty['quick'] !== '' ) {
					$curr_new_set = false;
					if ( $curr_warranty['quick'] == 'included_warranty' && isset( $curr_warranty['included_warranty'] ) ) {
						$curr_fields['warranty'] = $curr_warranty['included_warranty']['period'] . ' ' . self::hlp_get_warranty_string( $curr_warranty['included_warranty']['period'], $curr_warranty['included_warranty']['type'] ) . ' ' . __( 'included warranty', 'wcwar' );
						if ( !isset( $curr_fields['status'] ) ) {
							$curr_fields['status'] = '<a href="' . $admin_url . '" class="wcwar_badge in_warranty" title="' . __( 'Create Warranty Request for this Item', 'wcwar') . '">' . __( 'In Warranty', 'wcwar') . '</a>';
							$curr_fields['status'] .= '<a href="' . $admin_url . '" class="wcwar_badge warranty_button wcwar_create" data-ids="' . $curr_order . '|' . $curr_item_id . '" title="' . __( 'Create Warranty Request for this Item', 'wcwar') . '"><i class="wcwar-new"></i></a>';
						}
					}
						...
						if ( $curr !== 'no_warranty' ) {
							...
							$curr_fields['warranty'] = $curr_warranty['paid_warranty'][$curr]['period'] . ' ' . self::hlp_get_warranty_string( $curr_warranty['paid_warranty'][$curr]['period'], $curr_warranty['paid_warranty'][$curr]['type'] ) . ' ' . __( 'paid warranty', 'wcwar' ) . ' <small>(+ ' . wc_price( $curr_warranty['paid_warranty'][$curr]['price'] ) . ')</small>';
				...
				$curr_fields['warranty'] = __( 'Please refresh', 'wcwar' );
				$curr_fields['status'] = __( 'Please refresh', 'wcwar' );
			}
	?>
	...
	<?php
		...
		function war_request_warranty_column_header( $columns ) {
			...
			$columns['status'] = __( 'Request Status', 'your_text_domain' );
			return $columns;
		...
		function war_request_warranty_column( $column, $post_id ) {
			...
			if ( $column == 'status' ) {
				...
				if ( !empty( $curr_terms ) ) {
					...
					echo '<a href="' . get_permalink( $post_id ) . '" class="wcwar_badge warranty_button view_warranty" title="' . __( 'View Request Status', 'wcwar' ) . '"><i class="wcwar-view"></i></a>';
					echo '<a href="#" class="wcwar_badge warranty_button warranty_change_status" data-id="' . $post_id . '" title="' . __( 'Change Request Status', 'wcwar' ) . '"><i class="wcwar-change"></i></a>';
					echo '</span>';
				}
		...
		function war_request_metabox( $object, $box ) {
			...
			if ( $object->post_parent !== '0' || $curr_item == '-1' ) {
				...
				$str_type = __( 'Multiple Item Order', 'wcwar' );
			}
			else {
				$curr_parent_order = $object->ID;
				$str_type = __( 'Single Item Order', 'wcwar' );
			}
			...
			$metadata = function_exists( 'wc_get_order_item_meta' ) ? wc_get_order_item_meta( $curr_order, 'woocommerce_hidden_order_itemmeta' ) : $order->has_meta( $item_id );
		...
		<div id="woocommerce-order-items">
			<div class="war_warranty woocommerce_order_items_wrapper">
				...
				<p class="wcwar_warranty_details">
					<span class="wcwar_meta_order"><i class="wcwar-order"></i> <?php _e( 'Order ', 'wcwar' ); ?> #<?php echo $curr_order; ?> - <?php echo $str_type; ?><a href="<?php echo esc_url( admin_url( 'post.php?post=' . absint( $curr_order ) . '&action=edit' ) ); ?>" class="wcwar_badge warranty_button warranty_view_order" title="<?php _e( 'Manage Order', 'wcwar' ); ?>"><i class="wcwar-view"></i> Manage Order</a></span>
				</p>
				<p class="wcwar_warranty_status">
					<span class="wcwar_badge_parent"><i class="wcwar-icon"></i> <?php _e( 'Request Status', 'wcwar' ); ?>
					<?php
						...
						if ( !empty ( $curr_terms) ) {
							$switch_slug = $curr_terms->slug;
							if ( $switch_slug == 'new' ) {
								echo '<a href="' . esc_url( admin_url( 'post.php?post=' . absint( $curr_parent_order ) . '&action=edit' ) ) . '" class="wcwar_badge wcwar_change warranty_new">' . __( 'New', 'wcwar') . '</a>';
							}
							else if ( $switch_slug == 'processing' ) {
								echo '<a href="' . esc_url( admin_url( 'post.php?post=' . absint( $curr_parent_order ) . '&action=edit' ) ) . '" class="wcwar_badge wcwar_change warranty_processing">' . __( 'Processing', 'wcwar') . '</a>';
							}
							else if ( $switch_slug == 'completed' ) {
								echo '<a href="' . esc_url( admin_url( 'post.php?post=' . absint( $curr_parent_order ) . '&action=edit' ) ) . '" class="wcwar_badge wcwar_change warranty_completed">' . __( 'Completed', 'wcwar') . '</a>';
							}
							else if ( $switch_slug == 'rejected' ) {
								echo '<a href="' . esc_url( admin_url( 'post.php?post=' . absint( $curr_parent_order ) . '&action=edit' ) ) . '" class="wcwar_badge wcwar_change warranty_rejected">' . __( 'Rejected', 'wcwar') . '</a>';
							}
							if ( $curr_item !== '-1' ) {
								echo '<a href="' . esc_url( admin_url( 'post.php?post=' . absint( $curr_parent_order ) . '&action=edit' ) ) . '" class="wcwar_badge warranty_button" title="' . __( 'View Parent Request Status', 'wcwar') . '"><i class="wcwar-view"></i></a>';
							}
							echo '<a href="#" class="wcwar_badge warranty_button warranty_change_status" data-id="' . $curr_parent_order . '" title="' . __( 'Change Request Status', 'wcwar') . '"><i class="wcwar-change"></i></a>';
						}
					?>
					</span>
				</p>
				<p class="wcwar_email">
					<?php _e( 'Send a quick E-Mail the Customer', 'wcwar' ); ?> : 
					<textarea class="war_message"></textarea>
					<small><?php echo __( 'Use these variables in Emails') . ' : <em>%order_id%, %order_date%, %completed_date%, %customer_name%, %warranty_link%</em>'; ?></small>
					<span class="wcwar_badge wcwar_label"><?php _e( 'Email Template', 'wcwar' ); ?></span>
					<select class="war_email_selected">
						<option value=""><?php _e( 'None', 'wcwar' ); ?></option>
						<?php
					...
					<a href="#" class="wcwar_badge war_load"><?php _e( 'Load', 'wcwar' ); ?></a>
					<a href="#" class="wcwar_badge war_save"><?php _e( 'Save', 'wcwar' ); ?></a>
					<a href="#" class="wcwar_badge war_delete"><?php _e( 'Delete Selected', 'wcwar' ); ?></a>
					<a href="#" class="wcwar_badge war_send" data-ids="<?php echo $curr_order . '|' . $object->ID;?>"><?php _e( 'Send Email', 'wcwar' ); ?></a>
				</p>
				<p style="display:none">
					<label for="_wcwar_warranty_status">
					<?php _e( 'Change Request Status', 'wcwar' ); ?> : 
					<?php
				...
				<table cellpadding="0" cellspacing="0" class="woocommerce_order_items">
					<thead>
						<tr>
							<th class="item" colspan="2"><?php _e( 'Item', 'wcwar' ); ?></th>

							<?php do_action( 'woocommerce_admin_order_item_headers' ); ?>

							<th class="quantity"><?php _e( 'Qty', 'wcwar' ); ?></th>

							<th class="line_cost"><?php _e( 'Total', 'wcwar' ); ?></th>
	...
	?>
		<div id="woocommerce-order-items">
			<div class="war_warranty woocommerce_order_items_wrapper">
				...
				<p>
					<label for="_wcwar_warranty_order_id">
						<?php _e( 'Please enter Order ID', 'wcwar' ); ?> : 
						<input name="_wcwar_warranty_order_id" value="<?php echo $curr_order; ?>" />
					</label>
				</p>
			</div>
	...
	<?php
		...
		function war_ajax_email_send() {
			...
			if ( !empty( $order) ) {
				...
				$curr_subject = __( 'Warranty Request for order #', 'wcwar') . $_POST['order_id'] . ' - ' . get_bloginfo('name' );
				...
				if ( count( $curr_ordered) > 1 ) {
					...
					if ( empty( $curr_parent_req) ) {
						...
						$curr_parent_create = array(
							'post_title'    => __( 'Request for Order #', 'wcwar' ) . $_POST['order_id'],
							'post_content'  => __( 'This is a parent warranty request. Check child requests for details.', 'wcwar' ),
							'post_name'     => 'r_' . $_POST['order_id'],
							'post_status'   => self::$settings['wcwar_default_post'],
							'post_author'   => 1,
							'post_type'     => 'wcwar_warranty_req',
							'comment_status'=> 'open',
							'ping_status'   => 'closed'
					...
					$curr_create = array(
						'post_title'    => __( 'Request for Order #', 'wcwar' ) . $_POST['order_id'] . ' - ' . __( 'Item #', 'wcwar') . $_POST['item_id'],
						'post_content'  => '',
		...
		<?php
			...
			$id = method_exists( $curr_order, 'get_id' ) ? $curr_order->get_id() : $curr_order->ID;
				...
				if ( count( $curr_items) > 1 ) {
					...
					_e( 'Warranty Status (Multi Item Order):', 'wcwar' );
				}
				else {
					$curr_args = array(
						'post_type' => 'wcwar_warranty_req',
					...
					_e( 'Warranty Status (Single Item Order):', 'wcwar' );
				}
				echo '<br/>';
				...
				if ( !empty( $curr_req) ) {
					...
					if ( !empty ( $curr_terms) ) {
						$switch_slug = $curr_terms->slug;
						if ( $switch_slug == 'new' ) {
							echo '<a href="'.$curr_req[0]->guid.'" class="wcwar_badge wcwar_change warranty_new" title="' . __( 'View Request', 'wcwar') . '">' . __( 'New', 'wcwar') . '</a>';
							echo'<a href="' . get_edit_post_link( $curr_req[0]->ID ) . '" class="wcwar_badge warranty_button" title="' . __( 'View Request', 'wcwar') . '"><i class="wcwar-view"></i></a>';
						}
						else if ( $switch_slug == 'processing' ) {
							echo '<a href="'.$curr_req[0]->guid.'" class="wcwar_badge wcwar_change warranty_processing" title="' . __( 'View Request', 'wcwar') . '">' . __( 'Processing', 'wcwar') . '</a>';
							echo '<a href="' . get_edit_post_link( $curr_req[0]->ID ) . '" class="wcwar_badge warranty_button" title="' . __( 'View Request', 'wcwar') . '"><i class="wcwar-view"></i></a>';
						}
						else if ( $switch_slug == 'completed' ) {
							echo '<a href="'.$curr_req[0]->guid.'" class="wcwar_badge wcwar_change warranty_completed" title="' . __( 'View Request', 'wcwar') . '">' . __( 'Completed', 'wcwar') . '</a>';
							echo '<a href="' . get_edit_post_link( $curr_req[0]->ID ) . '" class="wcwar_badge warranty_button" title="' . __( 'View Request', 'wcwar') . '"><i class="wcwar-view"></i></a>';
						}
						else if ( $switch_slug == 'rejected' ) {
							$curr_fields['status'] = '<a href="'.$curr_req[0]->guid.'" class="wcwar_badge wcwar_change warranty_rejected" title="' . __( 'View Request', 'wcwar') . '">' . __( 'Rejected', 'wcwar') . '</a>';
					...
					echo '<a href="#" class="wcwar_badge warranty_button warranty_change_status" data-id="' . $curr_req[0]->ID . '" title="' . __( 'Change Request Status', 'wcwar') . '"><i class="wcwar-change"></i></a>';
				}
				else {
					echo '<span class="wcwar_badge warranty_notrequested" >' . __( 'This order has no warranty requests', 'wcwar') . '</span>';
				}
			...
			if ( isset( $_POST['war_submit'] ) && $_POST['war_submit'] == 'true' ) {
				...
				if ( !isset( $_POST['wcwar_return'] ) ) {
					...
					if ( $curr_multi_req == 'no' ) {
						...
						<?php
							echo __( 'Warranty already requested. View request status on this', 'wcwar') . ' <a href="'.$curr_req[0]->guid.'">' . __( 'link', 'wcwar') . '</a>.';
						}
						?>
							</p>
						<?php
					...
					if ( isset( $_POST['wcwar_qty'] ) ) {
						$request_content .= '<strong>' . __( 'Item was ordered multiple times, the requested number of item warranties is: ', 'wcwar' ) . intval( $_POST['wcwar_qty'] ) . '</strong>
';
					}
							...
							case 'checkboxes' :
								...
								foreach ( $cfld['field_options']['options'] as $cf ) {
									if ( isset( $_POST['wcwar_' . $cfld['cid'] . '_' . $i] ) ) {
										$request_content .= $cf['label'] . ' : ' . __( 'Checked', 'wcwar') . '
										';
									}
									$i++;
								}
...
';
					...
					$request_content = '';
					if ( isset( $_POST['wcwar_qty'] ) ) {
						$request_content .= '<strong>' . __( 'Item was ordered multiple times, the requested number of item returns is: ', 'wcwar' ) . intval( $_POST['wcwar_qty'] ) . '</strong>' . '
';
					}
					$request_content .= __( 'Nothing is wrong with the item. Customer just does not like it. Return was requested for this item.', 'wcwar') . '
';
					if ( isset( $_POST['wcwar_return_note'] ) ) {
						$request_content .= __( 'Return message from the buyer:', 'wcwar' ) . '
';
					...
					if ( count( $curr_ordered ) > 1 ) {
						...
						?>
							<div class="wcwar_warranty">
								<p class="wcwar_form_error">
									<span class="wcwar_info_icon"><i class="wcwar-cross"></i></span>
									<?php _e( 'There is no warranty or return policy for this item.', 'wcwar' ); ?>
								</p>
							</div>
						<?php
							$output = ob_get_clean();
						...
						?>
							<div class="wcwar_warranty">
								<p class="wcwar_form_error">
									<span class="wcwar_info_icon"><i class="wcwar-cross"></i></span>
									<?php _e( 'There is no warranty for this item.', 'wcwar' ); ?>
								</p>
							</div>
						<?php
							$output = ob_get_clean();
						...
						?>
							<div class="wcwar_warranty">
								<p class="wcwar_form_error">
									<span class="wcwar_info_icon"><i class="wcwar-cross"></i></span>
									<?php _e( 'Warranty for this item has expired.', 'wcwar' ); ?>
								</p>
							</div>
						<?php
							$output = ob_get_clean();
						...
						?>
							<div class="wcwar_warranty">
								...
								<?php
									if ( !empty( $curr_terms ) && $curr_terms->slug == 'completed' ) {
										echo __( 'This item has already been returned to the store.', 'wcwar' );
									}
									else if ( $curr_req[$return_id]->post_status == 'pending' ) {
										echo __( 'Return already requested but is still pending review. Please be patient.', 'wcwar' ) . '<br/>';
									}
									else {
										echo __( 'Return already requested. View request status on this', 'wcwar' ) . ' <a href="' . get_permalink( $curr_req[$return_id]->ID ) . '">' . __( 'link', 'wcwar') . '</a>.<br/>';
									}
								?>
								</p>
							</div>
						...
						?>
							<div class="wcwar_warranty">
								<p class="wcwar_form_error">
									<span class="wcwar_info_icon"><i class="wcwar-cross"></i></span><?php echo __( 'Warranty already requested. View request status on this', 'wcwar' ) . ' <a href="' . $curr_req[0]->guid . '">' . __( 'link', 'wcwar' ) . '</a>.'; ?>
								</p>
							</div>
						<?php
							$output = ob_get_clean();
							...
							?>
								<div class="wcwar_warranty">
									<p class="wcwar_form_error">
										<span class="wcwar_info_icon"><i class="wcwar-cross"></i></span><?php echo __( 'Warranty already requested. View request status on this', 'wcwar' ) . ' <a href="' . $curr_req_reverse[0]->guid . '">' . __( 'link', 'wcwar' ) . '</a>.'; ?>
									</p>
								</div>
							<?php
								$output = ob_get_clean();
...
/*
						...
						if ( self::hlp_valid_warranty( $curr_complete, $item) ) {
							echo $item['name'] . ' ' . __( 'warranty has expired.', 'wcwar' );
							return;
						}
						...
						if ( isset( $_POST['wcwar_return'] ) ) {
							...
							if ( $curr_returns == 'yes' && self::hlp_valid_return( $curr_complete, $item ) === false ) {
								echo $item['name'] . ' ' . __( 'return period has expired.', 'wcwar' );
								return;
							}
						}*/
						...
						?>
							<div class="wcwar_warranty">
								<p class="wcwar_form_error">
									<?php echo $item['name'] . ' ' . __( 'warranty has expired.', 'wcwar' ); ?>
								</p>
							</div>
						<?php
							$output = ob_get_clean();
						...
						$curr_create = array(
							'post_title'    => __( 'Request for Order #', 'wcwar' ) . esc_sql( $_POST['order_id'] ) . ' - ' . __( 'Item #', 'wcwar' ) . esc_sql( $_POST['item_id'] ),
							'post_content'  => esc_sql( $request_content ),
							'post_name'     => 'i_' . esc_sql( $_POST['order_id'] ) . '_' . esc_sql( $_POST['item_id'] ),
							'post_status'   => esc_sql( self::$settings['wcwar_default_post'] ),
							'post_author'   => 1,
							'post_type'     => 'wcwar_warranty_req',
							'comment_status'=> 'open',
							'ping_status'   => 'closed',
						...
						if ( isset( $_POST['wcwar_return'] ) ) {
							...
							?>
								<div class="wcwar_warranty">
									<p class="wcwar_form_error">
									<span class="wcwar_info_icon"><i class="wcwar-info"></i></span>
										<?php echo $item['name'] . ' ' . __( 'return period has expired.', 'wcwar' ); ?>
									</p>
								</div>
							<?php
							$output = ob_get_clean();
					...
					?>
						<div class="wcwar_warranty">
							<p class="wcwar_form_success">
								<span class="wcwar_info_icon"><i class="wcwar-check"></i></span><?php _e( 'Return request accepted. Go back to your order', 'wcwar' ); ?> <a href="<?php echo $order->get_view_order_url(); ?>"><?php _e( 'here', 'wcwar' ); ?></a>!
							</p>
						</div>
					<?php
					}
					...
					?>
						<div class="wcwar_warranty">
							<p class="wcwar_form_success">
								<span class="wcwar_info_icon"><i class="wcwar-check"></i></span><?php _e( 'Warranty request accepted. Go back to your order', 'wcwar' ); ?> <a href="<?php echo $order->get_view_order_url(); ?>"><?php _e( 'here', 'wcwar' ); ?></a>!
							</p>
						</div>
					<?php
					}
				...
				if ( is_user_logged_in() || isset( $guest_request ) ) {
					...
					?>
						<p class="wcwar_form_info">
							<span class="wcwar_info_icon"><i class="wcwar-info"></i></span>
					<?php
							echo __( 'Product is not selected. Please visit your account page at this', 'wcwar' ) . ' <a href="' . get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) . '">' . __( 'link','wcwar') . '</a>.';
					?>
						</p>
					<?php
						$output = ob_get_clean();
...
/*						$curr_multi_req = get_option( 'wcwar_enable_multi_requests', 'no' );
						...
						if ( $_GET['item_id'] == '-1' ) {
							...
							$form_out = '<p class="wcwar_item wcwar_return"><strong>' . __( 'Select item', 'wcwar' ) . '</strong>';
							...
							?>
								<p class="wcwar_form_error">
									<span class="wcwar_info_icon"><i class="wcwar-cross"></i></span>
									<?php _e( 'There is no valid warranty for any of the ordered items.', 'wcwar' ); ?>
								</p>
							<?php
								$output = ob_get_clean();
								return $output;
							...
							?>
								<p class="wcwar_form_info">
									<span class="wcwar_info_icon"><i class="wcwar-info"></i></span>
									<?php echo __( 'There is no warranty for this item.', 'wcwar' ) . ' ' . __( 'However you can still return it to the store as its return warranty is still valid.', 'wcwar' ); ?>
								</p>
							<?php
								$only_return = true;
							}
							...
							<div class="wcwar_warranty">
								...
								<?php
									if ( self::hlp_valid_return( $curr_complete, $item ) === true ) {
										...
										echo __( 'However you can still return it to the store as its return warranty is still valid.', 'wcwar' );
										$only_return = true;
									}
								?>
							</p>
						...
						if ( isset( $_GET['multiple'] ) && $_GET['multiple'] == '1' ) {
							...
							$form_out .= '<p class="wcwar_item wcwar_multiple"><strong>' . __( 'Select Quantity', 'wcwar' ) . '</strong><label for="wcwar_qty">';
							$form_out .= '<input id="wcwar_qty" name="wcwar_qty" type="number" min="1" max="' . $item['qty'] . '" value="1" />';
							$form_out .= '</label>';
							$form_out .= '<small>' . sprintf( __( 'Total number of items ordered is %1$s. Please select how many items will be included in your warranty request.', 'wcwar'), $item['qty'] ) . '</small>';
							$form_out .= '</p>';
						}

				?>
					<div class="wcwar_warranty woocommerce">
						<h2><?php printf( __( 'Order #<strong>%s</strong>', 'wcwar' ), $order->get_order_number() ); ?> - <?php echo $item['name']; ?></h2>
						<form action="<?php the_permalink(); ?>" method="POST">
							<p class="order-info">
							<?php
								printf(
									__( 'Order #%s was placed on %s and is currently %s. To request a warranty for <em>%s</em> please fill in the form bellow.', 'wcwar' ),
						...
						?>
							<p class="wcwar_form_error">
								<span class="wcwar_info_icon"><i class="wcwar-info"></i></span><?php _e( 'Please fill in the required fields.', 'wcwar' ); ?>
							</p>
						<?php
						}
					...
					?>
						<p class="wcwar_return">
							<strong><?php _e( 'Return is still available', 'wcwar' ); ?></strong>
							<label for="wcwar_return">
								<input type="checkbox" id="wcwar_return" name="wcwar_return" /> <?php _e( 'Return item', 'wcwar' ); ?>
							</label><br/>
							<small><?php _e( 'Item returns are available in this shop. If you just want to return an item to the store please check this option.', 'wcwar' ); ?></small>
						</p>
						<p class="wcwar_return_note">
							<strong><?php _e( 'Why are you returning this item?', 'wcwar' ); ?></strong>
							<label for="wcwar_return_note">
								<textarea name="wcwar_return_note"></textarea>
							</label>
							<small><?php _e( 'Enter additional information for this return request.', 'wcwar' ); ?></small>
						</p>
					<?php
						}
						...
						foreach ( $curr_fields['fields'] as $cfld ) {
							if ( $cfld['required'] === true ) {
								$req = '<span class="wcwar_required">' . __( 'Required', 'wcwar') . '</span>';
							}
							else $req = '';
					?>
						...
						?>
							<label for="wcwar_<?php echo $cfld['cid']; ?>_address" class="double"><?php _e( 'Address', 'wcwar' ); ?>
								<input type="text" id="wcwar_<?php echo $cfld['cid']; ?>_address" name="wcwar_<?php echo $cfld['cid']; ?>_address" />
							</label>
							<label for="wcwar_<?php echo $cfld['cid']; ?>_city" class="single"><?php _e( 'City', 'wcwar' ); ?>
								<input type="text" id="wcwar_<?php echo $cfld['cid']; ?>_city" name="wcwar_<?php echo $cfld['cid']; ?>_city" />
							</label>
							<label for="wcwar_<?php echo $cfld['cid']; ?>_zip" class="single"><?php _e( 'Zipcode', 'wcwar' ); ?>
								<input type="text" id="wcwar_<?php echo $cfld['cid']; ?>_zip" name="wcwar_<?php echo $cfld['cid']; ?>_zip" />
							</label>
							<label for="wcwar_<?php echo $cfld['cid']; ?>_state" class="single"><?php _e( 'State / Province / Region', 'wcwar' ); ?>
								<input type="text" id="wcwar_<?php echo $cfld['cid']; ?>_state" name="wcwar_<?php echo $cfld['cid']; ?>_state" />
							</label>
							<label for="wcwar_<?php echo $cfld['cid']; ?>_country" class="single"><?php _e( 'Country', 'wcwar' ); ?>
					...
					?>
						...
						?>
							...
							<input value="<?php _e( 'Submit Request', 'wcwar' ); ?>" type="submit" class="button" />
						</p>
						</form>
					</div>
				<?php
					...
					?>
						<div class="wcwar_warranty">
							<p class="wcwar_form_error">
								<span class="wcwar_info_icon"><i class="wcwar-cross"></i></span>
								<?php _e( 'You cannot make this request.', 'wcwar' ); ?>
							</p>
						</div>
					<?php
					}
			...
			?>
				<div class="wcwar_warranty woocommerce">
					<form action="<?php the_permalink(); ?>" method="POST">
					<p class="wcwar_form_info">
						<span class="wcwar_info_icon"><i class="wcwar-info"></i></span> <?php _e( 'Warranty requests for users that are not logged in are allowed. Please fill in the form bellow to request a warranty for you order.', 'wcwar' ); ?>
					</p>
					<p>
			...
			<?php
				...
				?>
					<div class="wcwar_warranty">
						<p class="wcwar_form_error">
							<strong><?php _e( 'Guest warranty requests are not allowed!', 'wcwar' ); ?></strong>
	...
	<?php
		...
		function war_email( $order, $sent_to_admin, $plain_text ) {
			...
			<h3><?php _e( 'Available Warranties for this Order', 'wcwar' ); ?></h3>
			<?php
				if ( !in_array( $order->get_status(), apply_filters( 'wcwar_warranty_order_status', array( 'completed' ), $order ) ) ) {
			?>
			<p>
				<?php _e( 'Your warranties will be available once your order is complete.', 'wcwar' ); ?>
			</p>
			<?php
				$curr_complete = get_post_meta( $id, '_ordered_date', true );
				...
				<?php
					...
					if ( isset( $curr_warranty['quick'] ) && $curr_warranty['quick'] !== '' ) {
						if ( $curr_warranty['quick'] == 'included_warranty' && isset( $curr_warranty['included_warranty'] ) ) {
					?>
							<h3><?php echo '#' . $i . ' ' .$item['name'] . ' ' . __( 'Warranty', 'wcwar') . ' - ' . __( 'Included Warranty', 'wcwar' ); ?></h3>
							<p>
								<?php if ( isset( $curr_warranty['included_warranty']['thumb'] ) ) { ?>
									<img width="64" height="auto" src="<?php echo $curr_warranty['included_warranty']['thumb']; ?>" />
								<?php } ?>
								...
								<?php
									if ( $curr_notallowed === false ) {
										...
										$curr_request = esc_url( add_query_arg( $curr_args, get_permalink( self::wpml_get_id( get_option( 'war_settings_page' ) ) ) ) );
										echo $item['name'] . ' ' . __( 'warranty is still valid. Request your warranty at this', 'wcwar') . ' ' . '<a href="' . $curr_request . '" class="wcwar_request">' . __( 'link', 'wcwar') . '</a>.';
									}
								?>
							</p>
					<?php
		...
		function war_cart_price( $val, $attr, $content = null ) {

			if ( isset( $attr['wcwar_pa_price'] ) ) {
				self::$settings['add_msg'] = true;
				return $val . '<small> *(+ ' . wc_price( $attr['wcwar_pa_price'] ) . ')</small>';
			}
		...
		function war_cart_help() {
			if ( isset( self::$settings['add_msg'] ) ) {
				printf( '
					...
					</tr>', __( 'Your purchase has included paid warranties!', 'wcwar' ) );
			}
		}
