			...
			<?php
				...
				if ( !empty($curr_customizations) ) {
					...
					for ( $i = 0; $i < count($curr_customizations['ivpa_attr']); $i++ ) {
						...
						$html .= '<label><span>' . __( 'Override Attribute Name', 'ivpawoo' ) . '</span> <input type="text" name="ivpa_title[' . $i . ']" value="' . esc_attr( stripslashes( $curr_customizations['ivpa_title'][$i] ) ) . '" /></label>';

						$html .= '<label><span>' . __( 'Add Attribute Description' ,'ivpawoo' ) . '</span> <textarea name="ivpa_desc[' . $i . ']">' . stripslashes( $curr_customizations['ivpa_desc'][$i] ) . '</textarea></label>';
